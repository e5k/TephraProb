...
for i = 1:length(s)
    ...
    data = ge_imagesc(XX(1,:), flipud(YY(:,1)) ,double(obj.CData),...
        'imgURL',fullfile(project.run_pth, 'KML', [strParts{1},'_',strParts{2}, '.png']),...
        'cLimLow',ax.CLim(1),...
        'cLimHigh',ax.CLim(2),...
    ...
    cntr = ge_contour(XX, YY ,double(obj.CData),...
        'lineValues',ctVal,...
        'lineColor', '000000',...
        'lineWidth', 2,...
        'cLimLow',ax.CLim(1),...
...
cbar = ge_colorbar(max(XX(1,:)), min(YY(:,1)) ,double(obj.CData),...
    'numClasses',9,...
    'cLimLow',ax.CLim(1),...
    'cLimHigh',ax.CLim(2),...
    'cBarFormatStr','%+01.2f',...
...
box = ge_box(min(min(XX)), max(max(XX)), min(min(YY)), max(max(YY)), ...
    'polyColor', '000000FF',...
    'lineColor', 'FF000000',...
    'name', 'Comptation domain');
...
toWrite = [GEfold; {cbar}; {box}; {pt}];
target = fullfile(project.run_pth, 'KML', [project.run_name, ' - ', ttlLab(1:end-3), '.kml']);
ge_output(target,[toWrite{:}], 'name', [project.run_name, ' - ', ttlLab(1:end-3)])
